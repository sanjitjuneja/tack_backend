...
def test_phone_registration(anon_client, phone_number, db, new_user, mocker):
    ...
    mocker.patch(
        "socials.sms_service.TwilioClient.send_signup_message",
        return_value="SM123123123",
    )

    response = anon_client.post(
        reverse("signup-send-code"), {"phone_number": phone_number}
    )
    ...
    response = anon_client.post(
        reverse("verify-sms-code"), {"sms_code": sms_code, "uuid": uuid}
    )
    assert response.status_code == 200
...
def test_login(anon_client, db, user_creds):
    response = anon_client.post(
        ...
        {"phone_number": user_creds["phone_number"], "password": user_creds["password"]},
    )
