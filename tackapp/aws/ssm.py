...
def receive_setting_parameters(aws_access_key_id: str, aws_secret_access_key: str, aws_region: str) -> tuple:
    ...
    response = client.get_parameters(
        Names=['ALLOWED_HOSTS', "CELERY_BROKER", "CHANNEL_LAYERS_HOST"],
        WithDecryption=True
    )
    allowed_hosts, celery_broker, channel_layers_host = response.get("Parameters")
    return allowed_hosts, celery_broker, channel_layers_host
